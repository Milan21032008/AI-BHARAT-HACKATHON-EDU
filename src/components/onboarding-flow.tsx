...
export function OnboardingFlow() {
  ...
  const { completeOnboarding, login, setSelectedTeacher } = useAppContext();
  ...
  // Strict Details Validation for Step 2
  ...
  // Strict Final Validation for Step 5
  const isFinalStepValid = () => {
    if (formData.role === "Child") {
      return !!(formData.gradeClass);
    }
    if (formData.role === "Parent") {
      return !!(formData.childName && formData.childName.trim() !== "");
    }
    if (formData.role === "Teacher") {
      return !!(formData.schoolName && formData.schoolName.trim() !== "" && formData.subject);
    }
    return false;
  };

  return (
    <div className="w-full max-w-md mx-auto py-8 space-y-8 animate-fade-in">
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-primary/10 shadow-2xl p-8 min-h-[520px] flex flex-col transition-all duration-500">
             ...
             <div className="flex-1 space-y-5 overflow-y-auto max-h-[420px] pr-2 pb-4 scrollbar-hide">
                <div className="space-y-1.5">
                  ...
                  <Input 
                    ...
                    className="h-12 rounded-xl font-semibold border-primary/10 focus:border-primary/30"
                    value={formData.name || ""}
                ...
                <div className="space-y-1.5">
                  <Label className="text-[10px] font-black uppercase tracking-widest opacity-60 ml-1">Phone or Email</Label>
                  <div className="relative">
                    <Mail className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground/40" />
                    <Input 
                      placeholder="email@school.com or phone" 
                  ...
                  <div className="relative">
                    ...
                    <Input 
                      ...
                      className={cn(
                        ...
                        confirmPassword && !passwordsMatch && "border-destructive/30 focus:border-destructive"
                      )}
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.language} onValueChange={(val) => setFormData({...formData, language: val})}>
                    ...
                    <SelectContent className="rounded-2xl border-none shadow-2xl">
                      {languages.map(l => <SelectItem key={l} value={l} className="font-medium">{l}</SelectItem>)}
                    </SelectContent>
                  </Select>
                </div>
             </div>
        ...
        {step === 3 && (
          <div className="space-y-8 flex-1 flex flex-col items-center justify-center animate-fade-in">
             ...
             <div className="space-y-6 w-full max-w-[280px]">
                <Input 
                    placeholder="000 000" 
                    className="h-16 text-center text-3xl font-black tracking-[0.5em] rounded-2xl border-primary/20 bg-primary/5"
                    maxLength={6}
                    value={otp}
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                        ...
                        <Select value={formData.gradeClass} onValueChange={(v) => setFormData({...formData, gradeClass: v})}>
                            ...
                            <SelectContent className="rounded-2xl border-none shadow-2xl">{classes.map(c => <SelectItem key={c} value={c} className="font-medium">{c}</SelectItem>)}</SelectContent>
                        </Select>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <Select value={formData.subject} onValueChange={(v) => setFormData({...formData, subject: v})}>
                            ...
                            <SelectContent className="rounded-2xl border-none shadow-2xl">
                              {subjects.map(s => <SelectItem key={s} value={s} className="font-medium">{s}</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </div>
                  </div>
          ...
          <div className="space-y-6 flex-1 flex flex-col animate-fade-in">
             ...
             <div className="flex-1 space-y-6">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="link" className="h-auto p-0 text-[10px] font-bold text-primary uppercase tracking-widest" onClick={goToForgotPassword}>
                      Forgot Password?
                    </Button>
                  </div>
                  <div className="relative">
             ...
             <div className="flex gap-3 mt-auto pt-6 border-t border-primary/5">
                ...
                <Button 
                  ...
                  disabled={isSubmitting || !loginData.identifier || !loginData.password} 
        ...
        {step === 7 && (
          <div className="space-y-6 flex-1 flex flex-col animate-fade-in">
             ...
             <div className="flex gap-3 mt-auto pt-6 border-t border-primary/5">
                <Button variant="ghost" className="h-12 rounded-xl font-bold" onClick={backToLogin}>
                  <ArrowLeft className="size-4 mr-2" />
                  Back
                </Button>
                <Button 
                  className="flex-1 h-12 rounded-xl font-bold shadow-xl shadow-primary/20" 
                  disabled={isSubmitting || !forgotPasswordIdentifier} 
